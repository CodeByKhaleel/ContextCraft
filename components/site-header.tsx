...
import { BookOpen, Bot, GitBranch, Menu, Search, Workflow, X } from "lucide-react";
...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#02030d]/95 px-3 pt-2 backdrop-blur sm:px-5 lg:px-8">
      <div className="mx-auto max-w-[1440px] rounded-lg border border-cyan-300/35 bg-slate-950/60 shadow-[0_0_0_1px_rgba(255,255,255,0.06),0_0_28px_rgba(14,165,233,0.35),inset_0_0_22px_rgba(14,165,233,0.08)] backdrop-blur-2xl">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            type="button"
            className="grid h-10 w-10 place-items-center rounded-lg border border-white/10 bg-white/[0.045] text-slate-300 transition hover:border-cyan-300/50 hover:bg-cyan-300/10 hover:text-cyan-100 md:hidden"
        ...
        {mobileOpen ? (
          <nav className="grid gap-1 border-t border-white/10 p-2 md:hidden">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
